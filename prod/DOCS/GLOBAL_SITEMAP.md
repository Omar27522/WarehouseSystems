...
- `checkout.php`: Customer batch order checkout manifest verification, order backdating, and ownership transfer utility.
...
- `UI.php`: Dynamic template rendering for CSS styling loaders, custom dialog triggers, and toast notifications.
...
- `new_order.php`: Interactive order batch builder panel.
...
- `js/`: Modular javascript loaders matching the views (e.g. `checkout.js`, `warehouse.js`, `customer_registry.js`).
- `styles/`: View-specific styling sheets (e.g. `style.css`, `components.css`, `dialogs.css`, `warehouse.css`).
